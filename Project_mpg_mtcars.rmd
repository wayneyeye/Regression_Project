...
summary(fit_1)
```
All t-tests suggest signifant correlation in the varibles and factors we choosed. The residual & diagnostics plot to our fitted model also suggests no obvious pattern existed in residual (in SI). Below is a plot for better interpreting:  
...
g6<-ggplot(data=mtcars2,aes(x=wt,y=mpg))+
        ...
        ggtitle("Fitted model: mpg ~ TransmissionType*Weight + # Cylinder")
print(g6)
```
